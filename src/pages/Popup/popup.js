...
const defaultShortcut = navigator.userAgent.toUpperCase().includes("MAC")
  ? "Command+k"
  : "Control+k";
...
const getShortcut = () => {
  chrome.storage.sync.get("shortcut", (data) => {
    const sc =
      data && data.shortcut ? getShortcutName(data.shortcut) : defaultShortcut;
...
const updateShortcut = (shortcut) => {
  ...
  chrome.storage.sync.set({ shortcut: shortcut }, () => {
    getShortcut();
    hintText.innerText =
      "Changed successfully, refresh HackMD web page to activate";
  });
};
